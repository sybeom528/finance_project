...
final/timeseries_lib.py 인터페이스에 맞춰 재작성. final 단독 완결 — 시계열_Test
폴더 의존성 없음 (실행 시 final/data + final/phase3(data_outputs)/data 만 사용).
"""
...
def prepare_incremental_state(out_dir: str | Path,
    ...
    fallback_search_paths : list of Path | None, default None
        fold csv 가 out_dir 에 없을 때 검색할 경로 후보.
        None 시 자동 fallback (시계열_Test/Phase3_Robust_Extensions/data 시도).
        시계열_Test 폴더 없으면 full 학습. 한 번 복사된 후로는 final 만 사용.
    auto_truncate : bool
    ...
    # 1. fold csv 부재 시 — fallback 검색
    # 정책: final/phase3 에 fold csv 가 있으면 그대로 사용 (의존성 0).
    #       없으면 시계열_Test (있는 경우) 에서 1회 복사.
    #       시계열_Test 도 없으면 full 학습 (start_k=0, 모든 종목 처음부터).
    if not fold_path.exists():
        if fallback_search_paths is None:
            # 자동 fallback 검색 — 시계열_Test (있는 경우만)
            ts_path = _THIS_DIR.parent / '시계열_Test' / 'Phase3_Robust_Extensions' / 'data' / fold_name
            fallback_search_paths = [ts_path]
        ...
        for src in fallback_search_paths:
            ...
            if src.exists():
                if verbose:
                    print(f'  [copy] fold csv fallback 발견 → final 로 1회 복사')
                    print(f'         from: {src}')
                    print(f'           to: {fold_path}')
                    print(f'         (이후 실행은 final/phase3 만 사용 — 의존성 0)')
        ...
        if not copied:
            if verbose:
                print(f'  [check] fold csv 없음 (final, fallback 모두) → full 학습 필요')
                print(f'         학습 시 모든 종목 fold 0 부터 시작 (~6-8 시간)')
            return {
