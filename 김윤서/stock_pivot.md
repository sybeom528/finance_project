...
ch['beta_63d']  = rolling_beta(r, spy_ret, 63)
ch['beta_126d'] = rolling_beta(r, spy_ret, 126)
```
...
- `beta_63d` = 21×3 = 1거래분기. Sharpe, Sortino, Skew, Kurt 등 다른 모든 통계 측도와 윈도우를 통일해 비교 기준을 일관되게 유지.
- `beta_126d` = 21×6 = 1거래반기. 63d 대비 노이즈가 걸러진 중기 구조적 특성 포착.
- 두 값의 괴리가 크면 최근 종목 성격이 바뀌었다는 신호.

> **참고 — 기관 표준 베타와의 차이**: Bloomberg 기본 베타는 2년 주간 수익률(104주), Damodaran(NYU)은 60개월 수익률(5년)을 사용한다. 우리 `beta_63d/126d`는 이 기관 표준 베타가 아니라, **ML 피처로서 단기-중기 시장 민감도 변화를 포착하는 롤링 베타**다. (출처: [BYU Bloomberg Guide](https://guides.lib.byu.edu/bloomberg/Beta), [Damodaran beta 추정 논문](https://pages.stern.nyu.edu/~adamodar/pdfiles/papers/beta.pdf))

#### A-4. 금리 민감도 (1개)

```python
ch['rate_corr_63d'] = r.rolling(63).corr(fred_df['DGS10_chg5'])
...
63d = 1거래분기. 금리 정책은 통상 수개월 단위로 변화하므로 3개월 윈도우가 적절하며, `beta_63d`와 동일한 기간 기준으로 "같은 63일 구간에서 시장 민감도(beta)와 금리 민감도(rate_corr)를 동시에 비교"할 수 있다.

```
rate_corr_63d > 0.1  → 금리 상승 수혜 (XLF 계열)
rate_corr_63d < -0.2 → 금리 상승 피해 (채권, 유틸리티)
≈ 0                  → 금리 영향 중립
```
...
기존 `beta_63d`, `beta_126d`가 이미 이 역할을 한다. (Part A-3 참고)
...
def build_full_feature_panel(df_price, df_volume, fred_df, gdelt_df,
    ...
    feat['beta_63d']     = returns.apply(lambda r: r.rolling(63).cov(spy_ret) / spy_ret.rolling(63).var())
    feat['beta_126d']    = returns.apply(lambda r: r.rolling(126).cov(spy_ret) / spy_ret.rolling(126).var())
...
모든 피처(A~F, 거시, GDELT)는 **Step 3 ML 학습의 입력(Input)**으로 공통 사용된다.  
그러나 일부 피처는 ML 외에도 다른 파이프라인 단계에서 **추가적인 직접 역할**을 가진다.

```
[Step 2] 피처 엔지니어링
    ↓ 모든 피처
[Step 3] ML 학습 (XGBoost / TabPFN)
    → 출력: Q (기대수익 뷰), Ω (불확실성)
    ↓ Q, Ω
[Step 4] Black-Litterman
    → Prior π = λΣw_mkt + Q/Ω → μ_BL, Σ_BL
    ↓ μ_BL, Σ_BL
[Step 5] MVO 최적화
    → 성향별 비중 w*
```
...
| 태그 | 의미 |
|------|------|
| `ML입력` | XGBoost/TabPFN이 Q·Ω를 학습하는 입력 피처 (모든 피처 공통) |
| `레짐신호` | HMM 레짐 판별 피처 또는 레짐 전환 조기 경보 신호로 활용 |
| `BL-Ω보정` | ML이 학습에 반영하지만, BL 단계에서 Ω를 수동 조정할 때 직접 참조 가능 |
| `MVO제약` | Step 5 최적화 시 비중 상한·하한 제약 또는 종목 필터 조건으로 활용 |

> `ML입력`은 모든 피처에 해당하므로 아래 표에서는 **추가 역할만** 별도 표기

---

### 자산별 특성 피처 (총 30개)

| 파트 | 피처명 | 윈도우 | 의미 | 추가 사용 단계 | 세부 역할 |
|------|--------|--------|------|--------------|----------|
| A 기본 | `ret_1m/3m/6m/12m` | 21~252d | 모멘텀 | BL-Q방향 | ML이 높은 Q를 예측하는 핵심 신호 |
| A 기본 | `vol_21d`, `vol_63d` | 21/63d | 변동성 | BL-Ω보정 | 변동성 클수록 Ω 상향 — 예측 불확실성 반영 |
| A 기본 | `vol_ratio` | - | 변동성 가속도 | 레짐신호 | >1.5이면 위기 레짐 진입 가능성 — HMM 보조 |
| A 기본 | `beta_63d`, `beta_126d` | 63/126d | 시장 민감도 | MVO제약 | 공격형: beta 높은 종목 허용, 보수형: beta 낮은 종목 선호 |
| A 기본 | `rate_corr_63d` | 63d | 금리 민감도 | 레짐신호 | 금리 레짐(인상/인하 국면)별 종목 반응 분류 |
| A 기본 | `rsi_14d` | 14d | 과매수/과매도 | — | ML 입력 전용 (모멘텀 과열 필터) |
| B L1 | `sharpe_21d`, `sharpe_63d` | 21/63d | 위험 대비 수익 | BL-Ω보정 | Sharpe 높을수록 Ω 하향 — ML 예측에 더 높은 신뢰 부여 |
| B L1 | `sortino_63d` | 63d | 하락 위험 대비 수익 | BL-Ω보정 | 하방 변동성 낮은 종목 → Ω 하향 조정 근거 |
| B L1 | `ir_63d` | 63d | 섹터 대비 초과 수익 꾸준함 | — | ML 입력 전용 (Q 보조 신호) |
| C L2 | `skew_63d` | 63d | 왜도 (분포 비대칭) | BL-Ω보정 | 음의 왜도 클수록 실제 하락 위험 과소추정 → Ω 상향 |
| C L2 | `kurt_63d` | 63d | 첨도 (Fat Tail) | BL-Ω보정 | 첨도 높을수록 Σ가 위험 과소추정 → Ω 상향 |
| C L2 | `mdd_63d` | 63d | 최대 낙폭 | MVO제약 | MDD 임계값 초과 시 해당 종목 비중 상한 제약 가능 |
| D L3 | `avg_corr_63d` | 63d | 평균 상관계수 | 레짐신호 | 급등 시 상관관계 붕괴(위기 레짐) 조기 경보 — HMM 보조 |
| E L4 | `amihud_21d` | 21d | 유동성 | MVO제약 | 유동성 부족 종목 비중 상한 제약 (실제 매매 가능성 반영) |
| E L4 | `bb_pct` | 20d | 볼린저 밴드 위치 | — | ML 입력 전용 (과매수/과매도 위치 신호) |
| F L5 | `high52w_ratio` | 252d | 52주 신고가 대비 위치 | — | ML 입력 전용 (모멘텀 구조 신호) |
| F L5 | `ma_gap_20_60` | 20/60d | 이동평균 괴리율 | — | ML 입력 전용 (추세 기울기) |
| F L5 | `autocorr_21d` | 63d | 수익률 자기상관 | BL-Ω보정 | 자기상관 양수 → 모멘텀 지속 → ML 예측 신뢰도↑ → Ω 하향 |
| F L5 | `ivol_63d` | 63d | 고유 변동성 | BL-Ω보정 | 종목 특수 리스크 클수록 Q 신뢰도↓ → Ω 상향 |
| F L5 | `ret_rank_1m`, `ret_rank_3m` | 21/63d | 상대 수익률 순위 | — | ML 입력 전용 (크로스섹셔널 신호) |
| F L5 | `vol_rank` | 21d | 상대 변동성 순위 | MVO제약 | 보수형: vol_rank 낮은 종목 선호 — 성향별 필터 |
...
| 피처 | 추가 사용 단계 | 세부 역할 |
|------|--------------|----------|
| `VIX_level`, `VIX_contango`, `VIX_slope_*` | 레짐신호 | HMM의 핵심 입력 — 공포 레짐 판별 |
| `SKEW_level`, `SKEW_zscore` | 레짐신호 | 테일 리스크 급등 = 위기 레짐 선행 지표 |
| `HY_spread`, `HY_spread_chg` | 레짐신호 | 신용 스프레드 확대 = 위기 레짐 진입 신호 |
| `yield_curve`, `yield_curve_inv` | 레짐신호 | 장단기 금리차 역전 = 경기 침체 레짐 선행 |
| `claims_4wma`, `claims_zscore` | 레짐신호 | 고용 악화 = 경기 침체 확인 신호 |
| `sahm_indicator` | 레짐신호 | Sahm Rule 발동 = 공식 경기 침체 레짐 선언 |
| `WEI_level` | 레짐신호 | 주간 경제활동 지수 — 경기 레짐 추적 |
| `Cu_Au_ratio`, `Cu_Au_ratio_chg` | — | ML 입력 전용 (경기 민감도 지표) |
| `DGS10_chg5` | — | ML 입력 전용 (금리 변화 방향) |

> 거시 피처 중 레짐신호로 분류된 항목들은 **HMM의 p=7 관측 변수**로도 활용된다 (decision_log 17장).

### GDELT 감성 피처 (6개, 기존 유지)

| 피처 | 추가 사용 단계 | 세부 역할 |
|------|--------------|----------|
| `fin_sentiment`, `sent_momentum` | 레짐신호 | 뉴스 감성 급락 = 위기 레짐 보조 신호 |
| `sent_volatility`, `sent_zscore` | BL-Ω보정 | 뉴스 감성 불확실성 높을수록 Ω 상향 근거 |
| `news_spike`, `crisis_signal` | 레짐신호 | 뉴스 급증·위기 키워드 = HMM 위기 레짐 보조 |

### HMM 레짐 피처 (1개)

| 피처 | 사용 단계 | 세부 역할 |
|------|----------|----------|
| `hmm_crisis_prob` | ML입력 + BL-Ω보정 | 레짐 확률 자체를 ML 피처로 입력. BL 단계에서 crisis_prob > 0.7이면 전체 Ω를 스케일 업하는 조건부 보정에도 활용 가능 |

> decision_log 17장 설계 기준, p=7 (관측 변수 7개로 2-state HMM 학습)
...
```
┌──────────────────────────────────────────────────────────┐
│ Step 3: ML 학습 (모든 54개 피처 → XGBoost/TabPFN)         │
│   출력: Q (기대수익 뷰 벡터), Ω (불확실성 행렬)              │
└──────────────────────────────────────────────────────────┘
         │                          │
         ▼                          ▼
┌──────────────────┐     ┌──────────────────────────────┐
│ Step 4: BL        │     │ HMM 레짐 판별                 │
│                   │     │   주요 피처: vol_ratio,       │
│ • Q, Ω → μ_BL    │     │   avg_corr_63d, VIX_level,   │
│ • Ω 수동 보정:    │     │   HY_spread, yield_curve_inv, │
│   vol, ivol,      │     │   SKEW, claims, sahm,        │
│   skew, kurt,     │     │   news_spike, crisis_signal  │
│   autocorr,       │     │   → hmm_crisis_prob 출력      │
│   sharpe, sortino │     └──────────────────────────────┘
└──────────────────┘
         │
         ▼
...
│ Step 5: MVO 최적화                                         │
│   제약 조건 직접 사용:                                      │
│   • amihud_21d → 유동성 제약 (비중 상한)                   │
│   • beta_63d   → 성향별 beta 범위 제약                     │
│   • vol_rank   → 보수형 포트폴리오 종목 필터                │
│   • mdd_63d    → 최대 낙폭 임계 종목 제외                   │
└──────────────────────────────────────────────────────────┘
```
