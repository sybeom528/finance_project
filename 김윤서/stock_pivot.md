...
## 10. LSTM extra_features 활용 가이드

> **배경 (2026-04-25 추가)**: Phase 1 LSTM(02_setting_A_daily21.ipynb) Run 결과 R²_OOS 음수, previous baseline(hit_rate 0.92) 대비 크게 부진. 원인 중 하나가 입력이 일별 log_return 단일 채널(input_size=1)이라 타겟(21일 누적 수익률)과 시간 스케일 불일치. 아래 피처들을 `build_fold_datasets(extra_features=...)` 인자로 추가하는 방향 검토.

### 그룹 1 — 즉시 사용 가능 (log_return만으로 계산)

> `raw_dict[ticker]['log_return']` 하나만 있으면 누수 없이 계산 가능.

| 피처 | 계산 | 이 문서 파트 | LSTM 추가 가치 |
|------|------|------------|--------------|
| `Y_trailing` | `log_ret.rolling(21).sum()` | — (§3 논의사항 추가) | **최우선 권장** — 타겟과 동일 스케일, previous baseline이 활용하는 자기상관 신호를 모델이 직접 관찰 |
| `mean_21` | `log_ret.rolling(21).mean()` | — | 일별 노이즈 제거, 저주파 추세 요약 |
| `std_21` | `log_ret.rolling(21).std()` | A `vol_21d` 기반 | 변동성 클러스터링 |
| `vol_ratio` | `vol_21d / vol_63d` | A-2 | 변동성 가속도, 레짐 전환 감지 |
| `sharpe_21d` | `mean/std × √252` | B-1 | 최근 수익 효율 |
| `sharpe_63d` | `mean/std × √252` | B-1 | 중기 수익 효율 |
| `sortino_63d` | 하락일 std만 사용 | B-2 | 하방 위험 대비 수익 |
| `skew_63d` | `log_ret.rolling(63).skew()` | C-1 | 수익률 분포 비대칭 (Fat Tail 신호) |
| `kurt_63d` | `log_ret.rolling(63).kurt()` | C-2 | 꼬리 두께 (극단 이벤트 빈도) |
| `autocorr_21d` | `rolling(63).apply(autocorr(lag=21))` | F-3 | 모멘텀 지속 vs 평균 회귀 구분 |

### 그룹 2 — 즉시 사용 가능 (Adj Close 필요, raw_dict에 있음)

> `raw_dict[ticker]['Adj Close']`가 이미 로드되어 있으므로 추가 데이터 수집 불필요.

| 피처 | 계산 | 이 문서 파트 | LSTM 추가 가치 |
|------|------|------------|--------------|
| `ret_1m` | `log(p / p.shift(21))` | A-1 | 1개월 모멘텀 |
| `ret_3m` | `log(p / p.shift(63))` | A-1 | 3개월 모멘텀 (가장 강한 알파 팩터) |
| `ret_6m` | `log(p / p.shift(126))` | A-1 | 중기 추세 |
| `ret_12m` | `log(p / p.shift(252))` | A-1 | 장기 추세 |
| `rsi_14d` | Wilder RSI(14) | A-5 | 과매수/과매도 구간 |
| `mdd_63d` | `price/rolling_max - 1` 의 min | C-3 | 모멘텀 약화, 투자 심리 반영 |
| `bb_pct` | `(p - lower) / (upper - lower)` | E-2 | 볼린저 밴드 위치, 평균회귀 신호 |
| `high52w_ratio` | `p / p.rolling(252).max()` | F-1 | 52주 신고가 대비 위치 |
| `ma_gap_20_60` | `MA20/MA60 - 1` | F-2 | 추세 기울기 (골든/데드크로스 강도) |

### 그룹 3 — 즉시 사용 가능 (Volume 필요, raw_dict에 있음)

| 피처 | 계산 | 이 문서 파트 | LSTM 추가 가치 |
|------|------|------------|--------------|
| `amihud_21d` | `|ret| / (price × volume)` rolling mean | E-1 | 유동성 (낮을수록 대형주 특성) |

### 그룹 4 — SPY 수익률 필요 (SPY 이외 종목에만 의미 있음)

> Phase 1은 SPY·QQQ 두 종목. SPY 자신의 beta to SPY는 항상 ≈1이라 무의미. QQQ 등 비-SPY 종목 LSTM에서 유효. Phase 2+ 개별 종목 단계에서 주력 사용.

| 피처 | 이 문서 파트 | 주의 |
|------|------------|------|
| `beta_60d`, `beta_120d` | A-3 | SPY 모델에서는 상수에 가까움 → QQQ, 개별 종목용 |
| `ivol_63d` | F-5 | SPY 잔차 ≈ 0 → QQQ, 개별 종목용 |

### 그룹 5 — LSTM 입력으로 사용 불가 (다중 종목 또는 외부 데이터 필수)

| 피처 | 이 문서 파트 | 불가 이유 |
|------|------------|---------|
| `ir_63d` | B-3 | 섹터 내 다른 종목 전체 동시 필요 |
| `avg_corr_63d` | D-2 | 전체 유니버스 동시 필요 |
| `ret_rank_1m/3m`, `vol_rank` | F-4 | 날짜별 전 종목 랭킹 (크로스섹셔널) |
| `rate_corr_60d` | A-4 | FRED DGS10 데이터 별도 로드 필요 |

---

### 단계별 적용 로드맵

| 단계 | 추가 피처 | input_size | 목적 |
|------|---------|-----------|------|
| **Phase 1 re-run** | Y_trailing, mean_21, std_21 | **4** | 시간 스케일 불일치 해소, previous baseline 신호 직접 제공 |
| **Phase 1.5** | + vol_ratio, rsi_14d, bb_pct, ma_gap_20_60, mdd_63d | **9** | 추세·과열·낙폭 신호 추가 |
| **Phase 1.5 확장** | + ret_1m/3m, sharpe_21d, skew_63d, kurt_63d | **14** | 모멘텀·위험조정 수익·분포 형태 |
| **Phase 2** (개별 종목) | + beta_60d/120d, ivol_63d, amihud_21d, autocorr_21d | **18+** | 시장 민감도·고유 위험·유동성 |

> **누수 검증 의무**: 새 피처 추가 시 §4 스타일 3단 검증 (assert + 육안 확인 표 + rolling NaN 처리) 필수. 모든 피처는 trailing rolling → 미래 참조 없음.

> **과적합 주의**: 피처를 늘릴수록 `hidden_size`를 줄여 파라미터/샘플 비율을 유지해야 한다. Phase 1 re-run 권장값: hidden=32, num_layers=1 (파라미터 ≈4,300개).
