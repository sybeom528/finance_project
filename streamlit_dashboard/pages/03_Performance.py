...
with col_w[0]:
    show_1y = st.checkbox("1년", value=True, key="perf_w1y")
with col_w[1]:
    show_3y = st.checkbox("3년", value=True, key="perf_w3y")
with col_w[2]:
    show_5y = st.checkbox("5년", value=False, key="perf_w5y")
...
st.caption(
    ...
    "Hold Out 24개월 (2024-2025) / 전체 (2010-2025)."
)
...
st.caption(
    ...
    "균형에 가까운 왜도 (✓ — SPY 음수 왜도 대비 우월), 높은 첨도 = fat tail (주식 일반 패턴), 꼬리 비율 > 1 = 상승 꼬리 우세 (✓). "
