...
> **개별주 제외 결정 논의 필요**: 생존편향(survivorship bias) 및 섹터 ETF와의 중복 노출 문제로 제외해야 하는지
-> 팀 회의 결과 우선 넣고 해보고, 문제가 커지면 제외하기로 결정

> **개별주와 ETF 동시 포함의 경우, 기존 처럼 prior(Black-Litterman)를 시가 총액 기준으로 분배할 수 없음. 1/n이나 risk parity를 고려해서 prior를 설정해야 함
...
> 1. 각 자산별로 모델링 하기에는 데이터의 수가 부족함

> 2. 위와 같이 패널 데이터로 구성한다 해도, 현재 개별 주가 변수(모멘텀, 변동성 등) 보다 거시 경제 지표들이 많아 제대로 된 예측이 될지에 대한 우려 존재
