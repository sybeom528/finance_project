...
def compute_pi(
    ...
    pi : pd.Series (n,)
        사전 균형수익률 벡터.
    lam : float
        위험회피 계수 = E[r_mkt - r_f] / σ²_mkt, clip 된 [0.5, 10.0].

    Notes
    -----
    - He-Litterman (1999) 표준 공식: π = λ · Σ · w_mkt
    - λ 는 시장 데이터로 동적 계산 (sigma2_mkt > 0 일 때).
      σ²_mkt = 0 fallback: λ = 2.5 (Black-Litterman 1990 가정).
    - λ clip [0.5, 10.0]: 음수/극단치 방지.
    """
...
# =============================================================================
# 6. 본 baseline 의 결정사항 (PLAN.md §2 일관)
# =============================================================================
# Q_FIXED = 0.003   # 월 0.3% = 연 3.6% (BAB factor 보수 추정 — Pyo & Lee 일관)
# TAU     = 0.05    # default. 추후 sensitivity 분석 가능
