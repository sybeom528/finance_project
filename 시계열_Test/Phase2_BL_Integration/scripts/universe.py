...
def get_shares_at(ticker: str, date: pd.Timestamp, shares_map: dict) -> float:
...
def get_adjusted_shares_at(
    ...
    """
    raw_shares = get_shares_at(ticker, date, shares_map)
...
def build_universe_history(
    ...
    # 1) 멤버십 캐시 (전체 기간 커버)
    history_start = pd.Timestamp(f'{earliest_year - 7}-01-01')
    history_end = pd.Timestamp(f'{latest_year + 1}-01-01')
    membership_cache = cache_dir / 'sp500_membership.pkl'
    membership = get_or_build_membership(history_start, history_end, membership_cache)

    # 2) 모든 멤버 union (캐시할 종목 목록)
    all_members = set()
    for member_set in membership.values():
        all_members.update(member_set)
    print(f'  [universe] 전체 unique 멤버: {len(all_members)} 종목')

    # 3) 종가 캐시 (전체 종목)
    prices_cache = cache_dir / 'prices_close_universe.pkl'
    prices = fetch_close_prices(
        all_members, history_start, history_end, prices_cache,
    )
    ...
    shares_map = fetch_shares_outstanding(
        all_members, history_start, shares_cache,
    )
