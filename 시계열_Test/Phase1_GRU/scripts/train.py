...
§3.6 — PyTorch 학습 루프 흔한 실수 10가지
    ...
    ✓ gradient clipping (GRU gradient explosion 방지)
§4.4 — GRU num_layers=1 dropout 함정 (models.py 에서 처리됨)

Notes
-----
학습 루프 자체는 Phase1_LSTM 의 train.py 와 동일하다.
GRU 와 LSTM 모두 nn.Module 서브클래스이므로 동일 인터페이스로 학습 가능.
"""
...
def train_one_fold(
    ...
    *,
    ...
    """단일 fold 에 대한 GRU 학습 루프.

    Parameters
    ----------
    model : nn.Module
        학습할 모델 (``GRURegressor`` 등). 함수 내부에서 ``.to(device)`` 호출.
