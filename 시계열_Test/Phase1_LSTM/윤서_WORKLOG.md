...
### 13차 Run 설계 결정: CEEMDAN H=1d (논문 원설계 재현)

**목적**: 논문(Su et al. 2026)의 원설계인 H=1d(내일 수익률 예측)를 SPY·QQQ에 동일하게 적용하여 결과 비교.

**변경 사항 (12차 대비)**:
- `horizon=5 → 1`, `PURGE=5 → 1`
- IMF 타깃: `rolling(5).sum().shift(-5)` → `shift(-1)` (rolling 불필요)
- 훈련 샘플: 730 → 734개/폴드
- 노트북: `02_setting_A_daily1_ceemdan.ipynb` (신규 생성)

**참조**: `논의사항/2026-04-28_결과분석13.md`

### 13차 Run 결과 (2026-04-28)

| 지표 | SPY | QQQ | 관문 | 판정 |
|---|---|---|---|---|
| hit_rate | 0.5534 ± 0.108 | 0.5900 ± 0.124 | > 0.55 | ✅ / ✅ |
| r2_oos | **-0.9188 ± 2.262** | **-0.7873 ± 2.652** | > 0 | ❌ / ❌ |
| pred_std/true_std | 0.915 | 0.892 | — | ✅ (정상) |
| best_epoch=1 | 0/82 (0%) | 0/82 (0%) | — | ✅ |
| **종합 판정** | **FAIL** | **FAIL** | — | ❌ |

n_folds=82, n_oos_samples=1,722

**Baseline 비교 (주목)**:
- previous R²: SPY -1.29, QQQ -1.26 (5d의 +0.55와 반대로 음수) → 1일 수익률의 평균회귀 특성
- train_mean R²: +0.002 (사실상 zero와 동일)
- QQQ train R²=-0.003: **인-샘플에서도 zero 예측보다 나쁨** → 1일 수익률에 학습 가능 패턴 자체가 없음
...
**실패 원인 (구조적)**:
1. **1일 수익률 = 백색 소음**: ACF ≈ 0, SNR ≈ 1/20. CEEMDAN 분해 후에도 IMF 다음 하루 값은 예측 불가
2. **5일 타깃의 겹침 효과**: 5일 rolling sum 인접 타깃 간 r≈0.8 — IS에서 trivial 패턴 학습 가능. 1일은 겹침 없음
3. **논문 데이터 차이**: 논문은 중국 A주식 (단기 추세추종 강함). SPY·QQQ는 고효율 ETF

**채택 결과 변경 없음**: Phase 1 최종 결과는 12차 Run(CEEMDAN 5d) 유지.
