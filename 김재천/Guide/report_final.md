...
> **최종 버전**: v4.1.1 (2026-04-17, **Step 11 추가**)
> **핵심 결론**: **M1_보수형_ALERT_B** (Sharpe 1.064, MDD -15.53%) — 경로 1(일별 경보→주식 축소) + MV 최적화 조합이 최우수
> **주요 발견**: 이론적으로 정교해 보이는 경로 2(HMM 레짐→Σ 전환)는 **두 차례 재설계 후에도 무효 확인**
...
### 🏆 최종 추천 전략: **M1_보수형_ALERT_B**

### 5.1 선정 근거 (4기준 Multi-criteria)

| 기준 | 값 | 가중치 |
|------|-----|------|
| Sharpe 1.064 | Top 3 | 0.35 |
| MDD -15.53% | **Top 1** | 0.25 |
| Calmar 0.788 | Top 5 | 0.20 |
| Sortino | Top 5 | 0.20 |
| **Multi-score** | **4.10 (1위)** | - |

### 5.2 v3 결과 (Sharpe 1.473)와 v4.1 결과 (1.064)의 차이 해명

**v3 (report_v3.md)**:
- Baseline: Equal Weight 1/30
- 경로 2 미구현 (개념 설계만)
- 거래비용 계산 방식 다름 (Step 7)

**v4.1 (report_v4.md, 본 문서)**:
- Baseline: **MV 최적화 (Step 4 재현)**
- 경로 2 완전 구현 + 재설계 (무효 확인)
- 60/40 벤치마크 비용 정교화 → 상대 Sharpe 기준 변화

**결론**: 두 수치는 **서로 다른 실험 조건**에서 도출된 값으로, 직접 비교 부적절. v4.1이 더 엄격한 조건(MV baseline + 정확한 비용).

### 5.3 이론-실증 괴리의 교훈

**경로 2 (레짐 Σ 전환) 실패의 3가지 원인**:
1. **Σ 차이가 MV 비중에 반영 미약** — Ledoit-Wolf 수축 + 30자산 분산 효과
2. **재최적화 비용 > 이론적 이득** — 월별 Σ 전환 시 누적 거래비용 과다
3. **경로 1과 정보 중복** — 경보 레벨이 이미 레짐 프록시

**시사점**: **이론적 정교함 ≠ 실증적 효과**. 단순성(경로 1)이 복잡성(경로 2)보다 우수.
