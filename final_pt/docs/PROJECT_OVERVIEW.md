...
> **LSTM σ 예측 + Black-Litterman 단일 view 프레임워크로 저변동 종목 중심의 위험성향별 펀드 후보를 구축한 quant 프로젝트.**
>
> 최종 갱신: 2026-05-20
...
2. EDA (횡단면)          ✅ 02b_LowVol_PortfolioSort.ipynb       — ANOMALY_ANALYSIS.md (저변동 그룹의 위험조정 우위)
3. LSTM HPO             ✅ 03a_LSTM_Optuna_GridSearch.ipynb     — V4_BEST_CONFIG 정당화 (Optuna 12-trial)
3. LSTM σ 예측           ✅ 03b_Volatility_Forecasting.ipynb     — LSTM + HAR + Diebold-Pauly ensemble (617 종목 각각 개별 학습)
4. Black-Litterman       ✅ 04_BL_Walkforward.ipynb (walk_forward, 90 슬롯)
5. MVO + 위험성향 매핑    ✅ 05b_Analyze.ipynb (조합 선택 + 통계 검정)
6. 3-레짐 안정성 + 민감도  ✅ 05b_Analyze.ipynb + 06_Regime_Analysis.ipynb
7. Streamlit 대시보드     ✅ streamlit_dashboard/ (7 페이지 완성) + vercel_deploy/ 소개 페이지
```
...
│   ├── bl_runner.py                   ← walk_forward 실행 엔진
│   ├── master_table.py                ← results/*.pkl → mt/rt 빌더 + winner 자동 식별
...
- **03b** 617 종목 각각 개별 walk-forward (225 fold): LSTM 0.5185 / HAR 0.3914 / **Ensemble 0.3822** (Best ensemble 403 종목, 65.3%)

### BL 백테스트 (K_CUT 2010-01 ~ 2023-12, 168개월)
- **가장 타당한 조합으로 선택** (자동 식별: `sortino_ir ≥ 10` 필터 + 전체기간 `sortino` 1위): `mat_eq_eq_raw_pap`
  - Winner: **Sharpe 1.096 / Sortino 1.826 / CAGR 16.25% / MDD −13.65%**
  - SPY: Sharpe 0.871 / Sortino 1.277 / CAGR 13.45% / MDD −23.93%
  - 차이: Sharpe +0.225, Sortino +0.549, CAGR +2.80%p, MDD +10.28%p (방어)
...
- **레짐 안정성**: 3-레짐 (R1 회복 / R2 확장 / R3 변동) HMM 기반, 전 구간 winner 상위 유지
