...
| `ff_factors_daily.csv` | FF5 + MOM 일별 | 02b §5 CAPM α 회귀 (참고용, 최종 메인 분석엔 미사용) |
| `ff5_monthly.csv` | FF5 + MOM 월별 | 보조 데이터 (최종 메인 분석엔 미사용) |
...
├── ff_factors_daily.csv    # FF5+MOM 일별 — 02b CAPM α 회귀 (참고용)
├── ff5_monthly.csv         # FF5+MOM 월별 — 보조 데이터
